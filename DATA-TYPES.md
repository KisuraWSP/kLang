...
`JSON` is a distinct immutable builtin type. `JSON(source : String)` parses JSON directly and is especially useful with here strings; other serializable values are recursively converted to JSON using alias field tags where present. `json_parse(source)` provides the non-throwing `Result[JSON, String]` parser. `json_stringify(value)` serializes JSON, Table, String-keyed Map, List, String, Char, Int, UInt, Float, Bool, Null, Option, enum, or alias struct values. `json_encode(value)` is its safe `Result[String,String]` form. `json_decode(source)` returns `Result[T,String]` and converts objects to deterministic insertion-ordered Tables, arrays to Lists, scalars to native values, and JSON null to Null. The `json` stdlib exposes these as `stringify`, `serialize`, and `deserialize` while retaining its older String encoder helpers.

`Parsable[T]` is an immutable metaprogramming value created from a source `String`, commonly a here string, plus an optional `List[String]` of source arguments. It exposes `source`, `original_source`, `ast : List[Table]`, `statement_count`, `runtime_type`, `runtime_info`, `cli_args`, `source_args`, combined `args`, `program`, `build_system`, `workspace`, `keywords`, and `message_poll`. `parsable_with_source`, `parsable_replace`, and `parsable_append` reparse changed source and return `Result[Parsable[T], String]`, preventing stale AST metadata. `parsable_source`, `parsable_ast`, `parsable_args`, `parsable_runtime_info`, and `parsable_workspace` are typed accessors. `parsable_begin_polling`, `parsable_poll_message`, and `parsable_intercept_message` provide a runtime message polling surface where `REQUEST_AST` messages are intercepted and answered with the stored AST. Keyword macros can call `macro_context()` to inspect invocation values and types, and can return `macro_expand(source)` to parse and execute generated Klang source at the call site.

`File(path : String)` creates an immutable native filesystem descriptor without opening the path. It exposes `.path`, `.name`, and `.extension`, plus `.read()`, `.read_lines()`, `.write(content)`, `.append(content)`, `.exists()`, `.size()`, `.create()`, and `.remove()`. The equivalent unbound functions are `file_read`, `file_read_lines`, `file_write`, `file_append`, `file_exists`, `file_size`, `file_create`, and `file_remove`. Operations are implemented entirely by the Go runtime and return `Result[..., String]`; filesystem failures are values rather than unchecked runtime failures. `String as File` creates the same descriptor, and `File as String` returns its path.

`OS()` creates an immutable descriptor for the current host and exposes `.name`, `.arch`, `.cpu_count`, `.path_separator`, `.path_list_separator`, and `.line_separator`. Native methods provide the current, temporary, and home directories; hostname and process ID; environment lookup and mutation; the complete environment map; working-directory changes; and subprocess execution. Fallible operations return `Result[..., String]`, missing environment variables return `None()`, and subprocess results are Tables containing `command`, `stdout`, `stderr`, `exit_code`, and `success`. Equivalent unbound operations use the `os_` prefix. The `os` stdlib module is the recommended facade and exposes readable UpperCamelCase functions plus typed process-result helpers; its former upper snake case names remain deprecated compatibility functions.
