...
44. File // Immutable native filesystem path descriptor whose I/O operations are implemented by the Go runtime
45. OS // Immutable native host descriptor whose process and operating-system operations are implemented by the Go runtime
46. Atom // Immutable identifier-shaped symbolic value intended for custom error codes and protocol states
...
Cast expressions with `as` are intentionally limited to builtin target types, builtin generic families such as `List[T]`, `Map[K,V]`, `Option[T]`, and `Result[T,E]`, child-width builtin types, and generic type variables. A type alias may be used as a cast target only when it resolves to one of those builtin forms. User-defined alias structs and enums are not valid cast targets.
...
Every struct-style alias value exposes `value.cast_as(TargetType)`. `Table` receives the public constructor fields under their declared field names, `JSON` recursively applies JSON tags, and `String` returns that JSON representation as text. A target struct alias is constructed by matching fields by name; missing fields use target defaults, while missing required fields and incompatible field types are compile-time errors. The result is an isolated value. This struct conversion API is separate from the builtin-only `value as Type` cast expression.

Standalone `#extend Type { ... }` declarations add statically checked methods to builtin types and struct-style alias types without changing their runtime representation. Extension method bodies receive an immutable `this` binding of the target type, may declare typed parameters and return values, and are called through ordinary selector syntax such as `10.days().ago()`. Extensions are additive: they cannot replace builtin protocol methods, alias-owned methods, or another extension with the same target and name. The interpreter and native JS backend both dispatch these methods by receiver type.

Alias functions may overload binary operators with `operator SYMBOL(other : Type) : ReturnType { ... }`. Supported symbols are `+`, `-`, `*`, `/`, `//`, `%`, `**`, `==`, `!=`, `>`, `>=`, `<`, and `<=`. Dispatch uses the left alias value as immutable `this` and passes the right value as the declaration's single required parameter. Comparison overloads must return `Bool`; arithmetic overload results are statically checked at their use site. Mutable `+=`, `-=`, `*=`, and `/=` bindings reuse the corresponding overload and require its result to be assignable back to the binding. Operator precedence remains fixed by the language and cannot be changed by an overload.

`JSON` is a distinct immutable builtin type. `JSON(source : String)` parses JSON directly and is especially useful with here strings; other serializable values are recursively converted to JSON using alias field tags where present. `json_parse(source)` provides the native non-throwing `Result[JSON, String]` parser. `json_stringify(value)` serializes JSON, Table, String-keyed Map, List, String, Char, Int, UInt, Float, Bool, Null, Option, enum, or alias struct values. `json_encode(value)` is its native safe `Result[String,String]` form. `json_decode(source)` returns `Result[T,String]` and converts objects to deterministic insertion-ordered Tables, arrays to Lists, scalars to native values, and JSON null to Null. The `json` stdlib facade converts those descriptive native failures into `Result[...,Atom]` codes such as `:json_parse_failed`, `:json_encode_failed`, and `:json_decode_failed`.

`Parsable[T]` is an immutable metaprogramming value created from a source `String`, commonly a here string, plus an optional `List[String]` of source arguments. It exposes `source`, `original_source`, `ast : List[Table]`, `statement_count`, `runtime_type`, `runtime_info`, `cli_args`, `source_args`, combined `args`, `program`, `build_system`, `workspace`, `keywords`, and `message_poll`. Native `parsable_with_source`, `parsable_replace`, and `parsable_append` return `Result[Parsable[T], String]`; the `language` stdlib facade converts them to `Result[Parsable[T],Atom]`. `parsable_source`, `parsable_ast`, `parsable_args`, `parsable_runtime_info`, and `parsable_workspace` are typed accessors. `parsable_begin_polling`, `parsable_poll_message`, and `parsable_intercept_message` provide a runtime message polling surface where `REQUEST_AST` messages are intercepted and answered with the stored AST. Keyword macros can call `macro_context()` to inspect invocation values and types, and can return `macro_expand(source)` to parse and execute generated Klang source at the call site.

`File(path : String)` creates an immutable native filesystem descriptor without opening the path. It exposes `.path`, `.name`, and `.extension`, plus `.read()`, `.read_lines()`, `.write(content)`, `.append(content)`, `.exists()`, `.size()`, `.create()`, and `.remove()`. The equivalent unbound native functions are `file_read`, `file_read_lines`, `file_write`, `file_append`, `file_exists`, `file_size`, `file_create`, and `file_remove`; these Go-backed operations return descriptive `Result[...,String]`. The `file` stdlib facade exposes the same operations as `Result[...,Atom]` with operation-specific codes. `String as File` creates the same descriptor, and `File as String` returns its path.

`OS()` creates an immutable descriptor for the current host and exposes `.name`, `.arch`, `.cpu_count`, `.path_separator`, `.path_list_separator`, and `.line_separator`. Native methods provide the current, temporary, and home directories; hostname and process ID; environment lookup and mutation; the complete environment map; working-directory changes; and subprocess execution. Native fallible operations return `Result[...,String]`, missing environment variables return `None()`, and subprocess results are Tables containing `command`, `stdout`, `stderr`, `exit_code`, and `success`. Equivalent unbound operations use the `os_` prefix. The recommended `os` stdlib facade converts failures to `Result[...,Atom]` and exposes readable UpperCamelCase functions plus typed process-result helpers.
...
Bulk integer I/O is available for data-intensive programs without introducing a new runtime type. `read_int()` reads one whitespace-delimited signed `Int`, `read_ints(count)` reads a `List[Int]`, and `print_ints(values)` writes one space-delimited output line. `interval_walk_max_scores(vertex_count, edge_count, data)` is the native interval-state graph primitive: `data` contains all vertex values followed by `(u, v, capacity, floor)` edge quadruples, and the returned `List[Int]` contains the maximum initial-state-plus-destination-value score for every start vertex, or `-1` when no edge can be traversed.

Iterable values expose lazy functional pipelines through selector methods. `filter`, `map`, `skip`, and `limit` append immutable execution stages without invoking callbacks or allocating intermediate Lists. `collect`, `sort`, `fold`, `first`, `any`, `all`, `for_each`, and the Iterator `.count` field are terminal operations that pull each source item through the complete stage sequence. Lists, Sets, typed Maps, Strings, Tables, count-compatible Int values, and existing Iterators can start a pipeline with `.iter()` or directly with a stage method. Map and Table pipelines yield deterministic `Table` entries containing `key` and `value`. `sort` is a materializing terminal because ordering requires buffering; stages before it remain fused into the buffer-building traversal. `limit` is the lazy short-circuiting stage; existing eager stdlib methods named `take` retain their original collection-returning behavior.

Generic type parameters may use named constraints in addition to explicit `restrict[...]` allow-lists. `T numeric` accepts numeric parent and child-width types, `T comparable` accepts primitive comparable values, `T hashable` accepts the safe primitive key space used by `Table` and `Set`, `T iterable` accepts values supported by `iter`/`len`, `T allocator_like` accepts builtin allocator/value wrappers, `T transferable` accepts values that may cross a spawn boundary or live inside Atomic, and `T TraitName` requires an `impl TraitName for ConcreteType` declaration.
...
`Option[T]` and `Result[T, E]` are the standard absence and fallibility types. `Some(value)`/`None()` construct options, and `Ok(value)`/`Err(error)` construct results. Results remain generic: a `Result[T, String]`, for example, must be inspected, matched, or recovered as a value. Postfix `!` is intentionally restricted to `Result[T, Atom]`; it unwraps `Ok(value)` or throws the error Atom. `throw` accepts only `Atom`, and every `catch` binding has type `Atom`. Helpers such as `option_map`, `option_unwrap_or`, `option_and_then`, `result_map`, `result_map_err`, `result_unwrap_or`, and `result_and_then` transform or recover values without unsafe `.value` access.

The `errors` stdlib module creates and compares error Atoms, constructs `Result[T,Atom]`, and converts `Result[T,String]` to a chosen stable code with `errors.WithCode`. The `exceptions` module throws Atoms, converts `Result[T,Atom]` through the same `!` path, and builds optional diagnostic Tables and formatted stack text only after an Atom has been caught. Mutable diagnostic metadata is never part of exception propagation.

Every implemented fallible stdlib facade uses `Result[...,Atom]`; this includes API/meta tooling, calendar/time, encoding, file/OS, hashing, host-interface descriptors, JSON, language source transforms, REPL parsing, and workspace validation. `Result[T,E]` remains generic for user code and generic transformation helpers. The native `raylib` facade is an effectful graphics API rather than a fallible `Result` facade: its typed Klang wrappers dispatch to raylib compiled into the host executable, validate runtime arguments, require window operations on the main runtime, and close an open window when program execution ends. Its primitive 2D surface includes window state and sizing, frame timing, keyboard and mouse input, colors, filled and outline shapes, text drawing and measurement, screenshots, random integers, and rectangle/circle/point collision queries. Resource handles for textures, fonts, audio, shaders, cameras, and models are intentionally deferred until the runtime has opaque lifecycle-managed native handle types. The `js-wasm` modules remain excluded until their JavaScript interop layer is implemented.

The focused stdlib facades deepen these core types without changing their runtime representation. `list` and `array` provide safe lookup and functional transforms; `table` provides immutable set/delete/merge, ordered views, filtering, mapping, and fallback composition; `option` and `result` provide named transforms, recovery, conversion, and throwing `expect` helpers. The `types` module supplies child-width aliases plus Kotlin-inspired extensions for numeric coercion/comparison/ranges, Bool conversion, String slicing/search/padding, Char classification, and immutable generic List access/slicing. The `strings`, `mathg`, `io`, `json`, `os`, and `test` modules provide deterministic string/math helpers, in-memory reader/writer operations, native JSON and host access, and reusable assertions.

Public stdlib namespaces use lowercase module names and UpperCamelCase functions, while receiver methods follow the language-native lower_snake_case style. Previous lower snake case, upper snake case, and PascalCase receiver aliases remain callable when compatibility requires them, but are marked `@deprecated` with a reason and replacement. The `api` module exposes the module catalog, convention metadata, source loading, Parsable AST inspection, polling, immutable source transforms, and structured migration suggestions. Canonical collection transforms use fused iterator pipelines where their signatures permit it.

Standard-library functions and methods may carry `@backend("JS")`, `@backend("WASM")`, or `@backend("Standalone")`. This metadata does not create a runtime value or alter the function type; it restricts availability, semantic body checking, runtime dispatch, and backend emission to the named target. Unannotated functions remain backend-portable.
...
Backend compilation uses a Go `Backend` contract with `Check`, `Emit`, and `Package` phases. The JS backend lowers the supported typed core to backend-neutral IR and emits JavaScript. Its value surface includes `Int`, `UInt`, `Float`, `Bool`, `String`, `Char`, recursively typed `List[T]` and `Map[K,V]`, and dynamic `Table`, with variables, arithmetic/comparison/boolean expressions, ordinary functions, namespaces, imported modules, single returns, `if`/`unless`, `while`, range loops, assignment, `break`/`continue`, `assert`, `throw`, and `print`. Qualified namespace names are flattened into collision-safe JavaScript identifiers, while selective module filters keep unused imported functions out of generated artifacts.

Backend support claims are defined by the machine-readable matrix in `src/engine/conformance/backend_features.json`. Each feature has a stable identifier and an explicit status for Standalone, JS, WASM packaging, and the bytecode VM. WASM `fallback` is distinct from bytecode `compiled` support. Unsupported backend diagnostics carry the corresponding feature identifier; adding or changing support requires updating the matrix and conformance tests.

The WASM backend reuses typed-core IR to emit versioned `KBC1` bytecode for `Int`, `UInt`, `Float`, `Bool`, `String`, and recursively supported `List[T]` values. Bytecode version 3 adds named function references and fused pipeline plans for `filter`, `map`, `skip`, `limit`, `collect`, `sort`, `fold`, `any`, `all`, and `for_each`. It also supports isolated List literals and bindings, checked indexing, direct indexed mutation, `len`/`.count`, integer range loops, `for_each` over Lists, Strings, and count-compatible Int values, local variables, arithmetic and comparison, short-circuit booleans, `if`, `while`, direct functions, single returns, assertions, and `print`. The Go WASM runtime executes this bytecode with a stack VM, indexed locals, checked jumps, source-line runtime errors, call-depth and instruction limits, and eager List cloning at value boundaries to preserve Klang isolation semantics. Projects using features outside this subset retain the browser interpreter path; the package manifest records `bytecode_status` as either `compiled` or `interpreter-fallback`.
...
JS-generated `Map[K,V]` values preserve declared key/value checks, checked reads, indexed and compound mutation, count, function-return conversion, and value isolation. `Table` values preserve primitive key kinds, insertion order, selector sugar, own-entry count, fallback lookup, deletion, key/value/entry helpers, sequence count, JSON conversion rules, and copy isolation. Generated equality is value-based for Char, List, struct aliases, Map, and Table values.
