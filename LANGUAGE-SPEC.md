...
- `BuildSystem` backend is restricted to `WASM`, `JS`, or `Standalone`. `Standalone` currently packages sources for the Go interpreter, `WASM` currently packages the browser-hosted Go runtime, and `JS` invokes experimental native JavaScript code generation.
...
- JS struct aliases support constructors, fields, trailing defaults, `#extend` methods, and value isolation across bindings and calls. `JSON(struct)` and `json_stringify(struct)` recursively honor JSON field tags and deterministic key ordering. Struct lifecycle/allocator hooks, nested traits, and nested implementations are rejected by the JS backend until equivalent runtime behavior exists.
