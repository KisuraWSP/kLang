...
Classic FizzBuzz control-flow example using loops, conditionals, and printing.
## Files
...
```sh
go run . check examples/fizzbuzz
```

Run the example through the interpreter:

```sh
go run . run examples/fizzbuzz
```

Package it as a browser WASM bundle:

```sh
go run . package examples/fizzbuzz --backend=WASM
```

Serve it directly with the built-in browser runtime server:

```sh
go run . serve examples/fizzbuzz --port=8080
```
